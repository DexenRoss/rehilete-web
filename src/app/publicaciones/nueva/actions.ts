...
const schema = z.object({
  ...
  workType: z.string().trim().max(80), subjectCreatorName: z.string().trim().max(180), externalUrl: optionalUrl, categoryId: z.string(), reviewerId: z.string(), tagIds: z.array(z.string()).default([]),
});
const nullable = (value: string) => value.trim() || null;

export async function createPublication(_: PublicationFormState, formData: FormData): Promise<PublicationFormState> {
  const parsed = schema.safeParse({ kind: formData.get("kind"), status: formData.get("status"), title: formData.get("title"), subtitle: formData.get("subtitle"), description: formData.get("description"), body: formData.get("body"), coverImageUrl: formData.get("coverImageUrl"), coverImageAlt: formData.get("coverImageAlt"), year: formData.get("year"), workType: formData.get("workType"), subjectCreatorName: formData.get("subjectCreatorName"), externalUrl: formData.get("externalUrl"), categoryId: formData.get("categoryId"), reviewerId: formData.get("reviewerId"), tagIds: formData.getAll("tagIds") });
  ...
  try {
    await prisma.publication.create({ data: { kind: data.kind, status: data.status, title: data.title, slug: `${slugify(data.title)}-${Date.now().toString(36)}`, subtitle: nullable(data.subtitle), description: nullable(data.description), body: data.body, coverImageUrl: nullable(data.coverImageUrl), coverImageAlt: nullable(data.coverImageAlt), year: data.year === "" ? null : data.year, workType: nullable(data.workType), subjectCreatorName: nullable(data.subjectCreatorName), externalUrl: nullable(data.externalUrl), categoryId: nullable(data.categoryId), reviewerId: data.kind === "REVIEW" ? nullable(data.reviewerId) : null, publishedAt: data.status === "PUBLISHED" ? new Date() : null, tags: { create: data.tagIds.map((tagId) => ({ tagId })) } } });
